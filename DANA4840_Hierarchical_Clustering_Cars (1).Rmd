...
single_dendrogram +
  ...
  plot_layout(ncol = 2)
```
Ward D and Ward D2 linkage methods show the highest height between clusters when they are joined, indicating good separability between the clusters. The Average linkage method also shows moderate separability between clusters. In contrast, the Complete linkage method demonstrates a greater disparity in the height at which clusters are joined, suggesting less uniformity in cluster separation. The Average linkage method typically has the lowest height for merging clusters, which implies that the clusters are less well-separated compared to the other methods.
...
single_cluster +
  ...
  plot_layout(ncol = 2)
```
When comparing the clusters visually, we observe that all linkage methods produce similar results except for the Complete linkage method. This suggests that Complete linkage provides a unique clustering outcome compared to the other methods. The fact that five out of the six methods yield similar results reinforces the reliability of the clustering result, indicating that the observed clustering pattern is robust and not solely dependent on the linkage method.
...
Connectivity: According to connectivity, the oprimal number of clusters is 2 as that is where it is lowest
...
Dunn Index: The highest value is for single linkage method at k=6
Connectivity: Average, single and ward linkage methods have the lowest connectivity at k=2, with single linkage method being lower than the rest consistently 
Silhouette width: Average, single and ward linkage methods have the highest Silhouette width at k=2, with average linkage method being higher than the rest consistintly
